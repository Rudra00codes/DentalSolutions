...
// Try to import SplitText plugin, fallback if not available
let GSAPSplitText: any = null;

// Only try to import SplitText on the client side
if (typeof window !== 'undefined') {
  try {
    GSAPSplitText = require('gsap/SplitText').SplitText;
  } catch (e) {
    // SplitText plugin not available, will use fallback
  ...
  useGSAP(
    () => {
      ...
      if (GSAPSplitText) {
        ...
        // Fallback: Simple word-by-word animation without SplitText plugin
        const words = text.split(' ');
        el.innerHTML = words.map(word => `<span class="split-word inline-block">${word}</span>`).join(' ');
